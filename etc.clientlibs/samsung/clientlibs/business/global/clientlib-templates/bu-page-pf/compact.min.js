...
(() => {
  ...
  class LocalMenuNavigation {
    constructor(el) {
      ...
      this.els = {
        ...
        mobileCTA: this.ele.find('.lnb-nav__cta-mobile a'),
        mobileCTABtn: this.ele.find('.lnb-nav__cta-inner'),
      };
    ...
    init() {
      ...
      $(window).on('resize', () => {
        ...
        if($(window).width() > 767) {
          this.$navParent.find(".lnb-nav__cta-mobile-wrapper").removeClass("show-cta");
        }
      });
    ...
    setElements() {
      ...
      this.$mobileCTA = this.$navParent.find(this.els.mobileCTA);
      this.$mobileCTABtn = this.$navParent.find(this.els.mobileCTABtn);
        ...
        if (this.type === 'default') {
          this.$defaultMenuItem.each((idx, el) => {
            if (el.hasAttribute('href')) {
              const targetEl = this.getTarget(el.getAttribute('href').trim());

              if (targetEl) {
                this.targets.push({ el, targetEl });
              }
            }
          });
        } else {
    ...
    bindEvents() {
      ...
      this.$mobileCTA.on('click', () => { this.showMobileCTA(); });
      this.$mobileCTABtn.on('click', () => { this.$navParent.find(".lnb-nav__cta-mobile-wrapper").removeClass("show-cta"); });
    ...
    scrollTo(target) {
      const scrollPos = (document.body.scrollTop || document.documentElement.scrollTop) + target.offset().top - this.$navParent.outerHeight();
      window.sg.common.lazyLoad.ignoreLazyLoadOnceUnder(document, scrollPos + this.$navParent.outerHeight());
    ...
    showMobileCTA() {
      this.$navParent.find(".lnb-nav__cta-mobile-wrapper").toggleClass("show-cta");
    }
  }
  ...
  const localMenuNavigation = {
    initAll() {
      [...document.querySelectorAll('.bu-nv-g-lnb')].forEach((el) => {
  ...
  function handleHash() {
    ...
    if (id.indexOf('#') >= 0 && id.length > 1) {
      const menuItem = $q(`.bu-nv-g-lnb .lnb-nav__item a[href="${id}"]`);

      if (menuItem.target.length) {
        const component = menuItem.closest('.bu-nv-g-lnb');
  ...
  $q.load(() => {
    setTimeout(() => {
      if ($q('.bu-nv-g-lnb .lnb-nav.lnb-nav--sticky').target.length || $q('.bu-nv-g-lnb .lnb-nav.lnb-nav--hide').target.length) {
        handleHash();
      }
    }, 100);
  });
